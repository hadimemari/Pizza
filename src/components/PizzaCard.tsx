...
const IndustrialLamp = memo(({ isOn }: { isOn: boolean }) => {
  return (
    <div className="flex flex-col items-center mb-2 lg:mb-4 relative">
      <div className="w-[1.5px] h-6 lg:h-8 bg-black/10 relative" />
  ...
  return (
    ...
    >
      <div 
        ref={cardRef}
        className={cn(
          "rounded-[3.5rem] w-full max-w-[90vw] sm:max-w-[420px] lg:max-w-[460px] h-[640px] sm:h-[680px] lg:h-[750px] flex flex-col overflow-hidden relative bg-white/40 backdrop-blur-3xl border border-white/20 shadow-[0_30px_100px_rgba(0,0,0,0.04)] transition-all duration-1000 will-change-transform z-10",
      ...
      >
        ...
        >
          <div 
            className="absolute w-[300px] h-[300px] -translate-x-1/2 -translate-y-1/2 transition-transform duration-150 ease-out rounded-full"
            style={{
              left: `${mousePos.x}px`,
              top: `${mousePos.y}px`,
        ...
        >
          ...
          <div className="w-1/2 h-full flex flex-col p-6 lg:p-8 justify-between">
            <div className="flex-1 flex flex-col space-y-3 lg:space-y-4 relative z-10 overflow-hidden">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">Premium</span>
                </div>

                <h2 className={cn(
                  "text-3xl lg:text-[2.6rem] font-black uppercase tracking-tighter transition-all duration-1000 leading-tight px-4 text-zinc-900",
                  !pizza.isAvailable && "opacity-40 grayscale blur-[1px]"
                )}>
                  {pizza.name}
                </h2>
                
                <p className={cn("font-black text-2xl lg:text-3xl mt-1", pizza.isAvailable ? "text-primary drop-shadow-[0_5px_15px_rgba(230,126,34,0.15)]" : "text-zinc-300")}>
                  {pizza.price}
                </p>
              </div>

              <p className="text-sm lg:text-[14px] leading-relaxed text-center px-4 text-zinc-600 font-medium max-w-[95%] mx-auto line-clamp-2">
                {pizza.description}
              </p>
              ...
              <div className="flex justify-center">
                <button 
                  ...
                  className="flex items-center gap-3 bg-black/[0.03] px-6 py-2 rounded-full text-[11px] font-bold text-zinc-900 hover:bg-primary/10 hover:text-primary transition-all group w-fit border border-black/[0.03]"
                >
            ...
            <Button 
              ...
              className={cn(
                "w-full h-14 lg:h-16 rounded-[2rem] text-white font-black text-lg transition-all mt-4 border-none shadow-lg relative overflow-hidden group/btn",
                pizza.isAvailable 
                  ? "bg-zinc-900 hover:bg-primary hover:scale-[1.02] active:scale-95" 
                  : "bg-zinc-200 text-zinc-400"
              )}
            >
              <div className="absolute inset-0 bg-white/10 translate-y-full group-hover/btn:translate-y-0 transition-transform duration-500" />
              <ShoppingCart className="mr-3 w-5 h-5 lg:w-6 lg:h-6 relative z-10" />
