...
async function execSqlIgnoreDuplicate(sql: string) {
  try {
    await db.$executeRawUnsafe(sql);
  } catch (e) {
...
async function createTables(log: string[]) {
  const statements = [
  ...
  for (let i = 0; i < statements.length; i++) {
    log.push(`  table/index ${i + 1}/${statements.length}`);
    await execSql(statements[i]);
  }
}

async function addForeignKeys(log: string[]) {
  // Use plain ALTER TABLE + JS error handling instead of DO $$ blocks
  // (DO blocks fail with PgBouncer transaction pooling)
  const fkeys = [
  ...
  for (let i = 0; i < fkeys.length; i++) {
    log.push(`  fkey ${i + 1}/${fkeys.length}`);
    await execSqlIgnoreDuplicate(fkeys[i]);
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    log.push("Tables created OK.");

    // Step 2: Add foreign keys
    log.push("Step 2: Adding foreign keys...");
    await addForeignKeys(log);
    log.push("Foreign keys OK.");

    // Step 3: Check if already seeded
    log.push("Step 3: Checking existing data...");
    ...
    log.push("No existing products, will seed.");

    // Step 4: Seed categories
    log.push("Step 4: Seeding categories...");
    ...
    log.push("Step 5: Seeding products...");
    const productsData = [
    ...
    log.push(`${productsData.length} products OK.`);

    // Step 6: Seed users
    log.push("Step 6: Seeding users...");
    ...
    // Step 7: Seed reviews
    log.push("Step 7: Seeding reviews...");
    ...
    return NextResponse.json(
      { error: "Setup failed", details: errMsg, log },
      { status: 500 }
    );
  }
}
